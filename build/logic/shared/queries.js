...
const defaultQueries = {
    ...
    rows: {
        ...
        defaultValue: fields => options.types[getCurType(fields)].searchNumRecords,
        secondary: true
    },

    sort: {
        value: fields => fields.sort,
        defaultValue: fields => Object.keys(options.types[getCurType(fields)].sorts)[0],
        secondary: true
    },
    ...
    filter: {
        ...
        defaultValue: () => "",
        searchTitle: (value, i18n) => i18n.format(i18n.gettext("Query: '%(query)s'"), { query: value }),
        filter: value => ({
            query_string: {
                query: value || "*",
    ...
    source: {
        ...
        searchTitle: (value, i18n) => models("Source").getSource(value).getFullName(i18n.lang),
    ...
    similar: {
        filters: {
            ...
            external: {
                ...
                match: () => {
                    const sourceIDs = models("Source").getSources().map(source => source._id);
                            ...
                            must: [{
                                ...
                                match: {
                                    "similarRecords.source": {
                                        query: sourceIDs.filter(id => id !== sourceID).join(" "),
                                        operator: "or"
                                    }
                                }
                            }]
