...
tap.test("getURL", { autoend: true }, t => {
    ...
    t.equal(batch.getURL(i18n.lang), "/artworks/source/test/admin?records=test/started", "Get URL");
});
...
tap.test("getError", { autoend: true }, t => {
    const batch = init.getRecordBatch();
    const errors = ["ABANDONED", "ERROR_READING_DATA", "ERROR_SAVING", "ERROR_DELETING"];
...
tap.test("setResults (with error)", t => {
    const error = ["Parse error on line 2:", "[    {        id: \"1234\",        ", "--------------^", "Expecting 'STRING', '}', got 'undefined'"].join("\n");

    const batch = init.getRecordBatch();
    const dataFile = path.resolve(process.cwd(), "testData", "default-error.json");
    batch.setResults([fs.createReadStream(dataFile)], err => {
        t.error(err, "Error should be empty.");
...
tap.test("processRecords", t => {
    const batch = init.getRecordBatch();
    const dataFile = path.resolve(process.cwd(), "testData", "default.json");
    const expected = [{ model: "test/1234", result: "unchanged", warnings: [] }, {
        model: "test/1235",
        result: "changed",
        warnings: [],
        diff: {
...
tap.test("RecordImport.advance", t => {
    ...
    const getBatches = callback => {
        RecordImport.find({}, "", {}, (err, batches) => {
            callback(null, batches.filter(batch => batch.state !== "error" && batch.state !== "completed"));
        });
    };
