...
for (const file of fs.readdirSync(typeFilterDir)) {
    if (file.indexOf(".js") >= 0) {
        typeFilterFiles[file] = fs.readFileSync(path.resolve(typeFilterDir, file));
    }
}
...
const bindStubs = () => {
    ...
    sandbox.stub(Record, "find", (query, callback, extra) => {
        ...
        if (query.$or) {
            ...
            matches = Object.keys(records).filter(id => records[id].source === query.source).map(id => records[id]);
        } else if (query.images) {
            matches = Object.keys(records).filter(id => records[id].images.indexOf(query.images) >= 0).map(id => records[id]);
        } else {
        ...
        if (!callback || extra) {
            const ret = {
                ...
                exec: callback => process.nextTick(() => callback(null, matches))
            };
            return ret;
        }
    ...
    sandbox.stub(Record, "count", (query, callback) => {
        const count = Object.keys(records).filter(id => !query.source || records[id].source === query.source).length;
    ...
    sandbox.stub(Record, "aggregate", (query, callback) => {
        ...
        const count = Object.keys(records).filter(id => records[id].source === source).length;
    ...
    sandbox.stub(Record, "fromData", (tmpData, i18n, callback) => {
        fromData.call(Record, tmpData, i18n, (err, record, warnings, creating) => {
    ...
    sandbox.stub(RecordImport, "fromFile", (fileName, source, type) => {
        const batch = recordImportFromFile.call(RecordImport, fileName, source, type);
        if (!batch.save.restore) {
            sandbox.stub(batch, "save", callback => batch.validate(err => {
                /* istanbul ignore if */
                if (err) {
    ...
    sandbox.stub(Image, "findOne", (query, callback) => {
        ...
        const id = Object.keys(images).find(id => images[id].hash === query.hash);
    ...
    sandbox.stub(Image, "count", (query, callback) => {
        const count = Object.keys(images).filter(id => !query.source || images[id].source === query.source).length;
    ...
    sandbox.stub(User, "findOne", (query, callback) => {
        const matches = users.filter(user => user.email === query.email || query._id && user._id.toString() === query._id.toString());
