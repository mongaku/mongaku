...
if (args.v || args.version) {
    ...
    const startCmd = [getBinary("naught"), "start", `--worker-count ${workers}`, `--ipc-file mongaku.ipc`, `--pid-file mongaku.pid`, `--log /dev/null`, `--stdout ${stdoutLog}`, `--stderr ${stderrLog}`, serverjs].concat(extraArgs).join(" ");
    ...
    const serverjs = localFile("../mongaku.js");
    const ignored = [path.join(localDir, "node_modules"), path.join(cwd, "node_modules")].join(",");

    const devCmd = [getBinary("supervisor"), `-w ${localDir},${cwd}`, "-e js", `-i ${ignored}`, "--", serverjs].concat(extraArgs).join(" ");
