...
const Filters = ({type, globalFacets, values}: Props, {options}: Context) => {
...
const SourceFilter = ({values, sources}: Props, {gettext}: Context) => (
...
const SimilarityFilter = ({queries, values}: Props, {gettext}: Context) => {
...
const ImageFilter = ({queries, values}: Props, {gettext}: Context) => {
...
const Sorts = ({values, sorts}: Props, {gettext}: Context) => (
    <div className="form-group">
        <label htmlFor="source" className="control-label">
            {gettext("Sort")}
        </label>
...
const SearchForm = (props: Props, {gettext, options, URL}: Context) => {
    ...
    const showImageFilter =
        typeOptions.hasImages && !typeOptions.requiresImages;
    ...
    return (
        <form action={URL(`/${type}/search`)} method="GET">
            ...
            {typeOptions.hasImageSearch && <SimilarityFilter {...props} />}
